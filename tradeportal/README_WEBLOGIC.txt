Weblogic requires some manual steps to setup tradeportal.

weblogic 10.3
1) create a new weblogic domain from scratch using 
<weblogic home>/common/bin/config.cmd

make sure to select a jdk version that matches what you built tradeportal with. I used SUN jdk 1.6.

2) JDBC resource setup
2a) create a mysql database named tradeportal.  Use tradeportal.sql to seed it. I created a user root, password passw0rd

2b)
-go into the weblogic admin console services->jdbc->data sources
-create a new datasource 
   name: jdbc/tradeportal
   URL: jdbc:mysql://localhost:3306/tradeportal
...
4) Once you've configured this, stick tradeportal.war into the autodeploy directory (ie - C:\bea\user_projects\domains\tradeportal\autodeploy)
**NOTE, this doesn't seem to work for weblogic 9.2, so deploy via the admin console
tradeportal will be deployed and you can browse to it at:

http://localhost:7001/tradeportal
